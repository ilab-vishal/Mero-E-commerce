...
from celery_service.tasks.process_product_update_webhook import process_product_update_webhook_task
...
setup_logging()
logger = logging.getLogger("webhook.shopify.product_update")

router = APIRouter()

@router.post("/products/update/{client_id}")
async def product_updated(
    ...
    logger.info("Received product update webhook request: %s", True if body else False)
    ...
    logger.info("Processing product update webhook request: %s", True if product else False)
    # Enqueue Celery task for async processing
    task = process_product_update_webhook_task.delay(
        client_id=client_id,
