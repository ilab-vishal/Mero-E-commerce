...
from celery_service.tasks.process_product_delete_webhook import process_product_delete_webhook_task
...
setup_logging()
logger = logging.getLogger("webhook.shopify.product_delete")

router = APIRouter()

@router.post("/products/delete/{client_id}")
async def product_deleted(
    ...
    logger.info("Received product delete webhook request: %s", True if body else False)
    ...
    logger.info("Processing product delete webhook request: %s", True if product else False)
    # Enqueue Celery task for async processing
    task = process_product_delete_webhook_task.delay(
        client_id=client_id,
